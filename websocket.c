  ...
  //      response_protocol = "null";
    ...
    if (ws_ctx->hybi > 0) {
        ...
        if (response_protocol) {
            snprintf(response, sizeof(response), SERVER_HANDSHAKE_HYBI_WITH_PROTO, sha1, response_protocol);
        } else {
            snprintf(response, sizeof(response), SERVER_HANDSHAKE_HYBI_WITHOUT_PROTO, sha1);
        }
    } else {
        if (ws_ctx->hixie == 76) {
